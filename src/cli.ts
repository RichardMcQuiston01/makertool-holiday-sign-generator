...
export interface CliArgs {
  ...
  readonly sayingFontPath: string;
...
export function parseArgs(argv: readonly string[]): Result<CliArgs, string> {
  ...
  const sayingFontPath = values['--saying-font'];
  ...
  if (!sayingFontPath) {
    return {
      ok: false,
      error: 'Missing required argument: --saying-font <path>',
    };
  }
  if (!outDir) {
...
function formatGenerateSignError(error: GenerateSignError): string {
  switch (error.stage) {
    ...
    case 'content':
      return `Content error: ${error.error.field}: ${error.error.message}`;
    case 'font':
...
export function runCli(
  ...
  let sayingFontBuffer: Buffer;
  try {
    sayingFontBuffer = deps.readFile(args.sayingFontPath);
  } catch (err) {
    return {
      ok: false,
      error: `Failed to read saying font file: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  const sayingFontResult = fonts.register(
    'saying-font',
    toArrayBuffer(sayingFontBuffer),
  );
  if (!sayingFontResult.ok) {
    return {
      ok: false,
      error: `Saying font: ${sayingFontResult.error.message}`,
    };
  }
