...
export function getGlyphOutline(
  ...
  // opentype.js can throw (rather than return) on fonts using OpenType
  // layout features it doesn't fully implement (seen with some GSUB
  // ccmp/ligature tables) — caught here so an unsupported font produces a
  // Result the caller can react to, not a process crash.
  let path: opentype.Path;
  try {
    path = glyph.getPath(0, 0, fontSizePx);
  } catch (err) {
    return {
      ok: false,
      error: {
        message: `Font "${font.familyName}" could not render "${character}": ${err instanceof Error ? err.message : String(err)}`,
      },
    };
  }
...
export function getTextOutline(
  ...
  // See the matching try/catch in getGlyphOutline above: opentype.js can
  // throw on fonts using OpenType layout features it doesn't fully support.
  let path: opentype.Path;
  try {
    path = opentypeFont.getPath(text, 0, 0, fontSizePx);
  } catch (err) {
    return {
      ok: false,
      error: {
        message: `Font "${font.familyName}" could not render "${text}": ${err instanceof Error ? err.message : String(err)}`,
      },
    };
  }
