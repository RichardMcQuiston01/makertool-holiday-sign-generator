...
function buildTestFontBuffer(): ArrayBuffer {
  ...
  const font = new opentype.Font({
    familyName: 'Holiday Sign Test Font',
    styleName: 'Regular',
...
describe('loadFont', () => {
  it('parses a valid font buffer', () => {
    ...
    if (result.ok) {
      expect(result.value.familyName).toBe('Holiday Sign Test Font');
...
describe('createFontRegistry', () => {
  it('registers and retrieves fonts by id', () => {
    ...
    if (getResult.ok) {
      expect(getResult.value.familyName).toBe('Holiday Sign Test Font');
    }
  });
